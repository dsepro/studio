...
export function UserManualModal({ isOpen, onClose, language }: UserManualModalProps) {
  ...
  const T = {
    ...
    headerBarItems: [
      ...
      language === 'zh-hk' ? '<strong>主題切換 (太陽/月亮/筆記型電腦圖示):</strong> 在淺色、深色和系統主題之間循環切換。' : '<strong>Theme Toggle (Sun/Moon/Laptop icon):</strong> Cycle through light, dark, and system theme.',
      language === 'zh-hk' ? '<strong>語言切換 (繁/EN 圖示):</strong> 切換顯示語言 (繁體中文/英文)。' : '<strong>Language Toggle (繁/EN icon):</strong> Toggle display language (Traditional Chinese/English).',
      ...
      language === 'zh-hk' ? '<strong>考試設定 (設定圖示):</strong> 開啟考試配置模式，您可以在其中手動輸入考試詳細資訊或從預設清單中選擇。' : '<strong>Exam Setup (Settings icon):</strong> Opens the exam configuration modal where you can manually enter exam details or select from a list of presets.',
    ...
    examInfoCardText: language === 'zh-hk' ? '顯示目前考試的詳細資訊，例如名稱、代號、科目、允許時間和說明。這些資訊可以透過「考試設定」進行配置或從預設集中選擇。' : 'Displays details about the current exam, such as title, code, subject, time allowed, and instructions. These can be configured via "Exam Setup" or chosen from a preset.',
    ...
    timerCardItems: [
      ...
      language === 'zh-hk' ? '<strong>開始/停止/重設:</strong> 控制計時器。重設操作是即時的，沒有確認提示。' : '<strong>Start/Stop/Reset:</strong> Control the timer. Reset is immediate without confirmation.',
    ],
    dataPersistenceTitle: language === 'zh-hk' ? '資料持久化' : 'Data Persistence',
    dataPersistenceText: language === 'zh-hk' ? '計時器設定（初始時長、目前剩餘時間、運行狀態）、考試詳細資訊和語言偏好都儲存在瀏覽器的本機儲存空間中。這表示即使您關閉瀏覽器分頁或重新整理頁面，您的設定也將保留。' : 'Timer settings (initial duration, current time left, running state), exam details, and language preference are saved in your browser\'s local storage. This means your settings will persist even if you close the browser tab or refresh the page.',
    ...
    tipsItems: [
      ...
      language === 'zh-hk' ? '如果計時器未運行，可以點擊時間顯示或使用 +/- 按鈕來快速修改剩餘時間。' : 'Quickly modify the remaining time by clicking on the time display itself (to open a modal) or using the +/- buttons if the timer isn\'t running.',
    ],
